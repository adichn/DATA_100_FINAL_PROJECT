...
air_quality_clean <- air_quality_data |>
  # Replace -200 values with NA since they appear to be missing data markers
  mutate(
...
## Model Plot 1
```{r}
model_plot1 <- ggplot(train_data, aes(x = `PT08.S1(CO)`, y = `CO(GT)`, color = Season)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    title = "CO Sensor vs Ground Truth CO by Season",
    x = "PT08.S1(CO) Sensor Reading (Standardized)",
...
val_metrics1 <- augment(model1, new_data = test_data) |>
...
model2 <- linear_reg() |>
  set_engine("lm") |>
  fit(`CO(GT)` ~ `PT08.S1(CO)` + `PT08.S5(O3)` + `NOx(GT)` + 
        Season + NOx_PT08_S1_Interaction, data = train_data)

# Calculate RMSE for model 2
val_metrics2 <- augment(model2, new_data = test_data) |>
  summarize(rmse = sqrt(mean((`CO(GT)` - .pred)^2)))
```
...
# Final model incorporating cleaned data and selected features
final_model <- linear_reg() |>
  set_engine("lm") |>
  fit(`CO(GT)` ~ `PT08.S1(CO)` + `NOx(GT)` + Season + 
    `PT08.S5(O3)`, data = bind_rows(train_data, test_data))

# Model diagnostics
diagnostics_plot1 <- augment(final_model, new_data = bind_rows(train_data, test_data)) |>
...
diagnostics_plot2 <- augment(final_model, new_data = bind_rows(train_data, test_data)) |>
...
final_diagnostics <- diagnostics_plot1 | diagnostics_plot2
final_diagnostics
```
